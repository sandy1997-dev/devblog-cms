    ...
    <div className="group relative my-6 rounded-xl overflow-hidden border border-ink-800/60 shadow-2xl shadow-black/20">
      <div className="flex items-center justify-between px-4 py-2.5 bg-ink-900/80 border-b border-ink-800/60">
        ...
        <div className="flex items-center gap-3">
          ...
          <CopyButton text={getRawText(children)} />
        </div>
      </div>
...
export const mdxComponents = {
  ...
  blockquote: ({ children, ...props }: React.BlockquoteHTMLAttributes<HTMLQuoteElement>) => (
    <blockquote {...props} className="my-6 border-l-4 border-amber-400 pl-6 py-1 not-italic">
  ...
  table: ({ children, ...props }: React.TableHTMLAttributes<HTMLTableElement>) => (
    <div className="my-6 overflow-x-auto rounded-xl border border-ink-200 dark:border-ink-800">
      <table {...props} className="min-w-full divide-y divide-ink-200 dark:divide-ink-800">{children}</table>
    </div>
  ),

  th: ({ children, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) => (
    <th {...props} className="bg-ink-50 dark:bg-ink-900 px-4 py-3 text-left text-xs font-ui font-semibold uppercase tracking-wider text-ink-600 dark:text-ink-400">{children}</th>
  ),

  td: ({ children, ...props }: React.TdHTMLAttributes<HTMLTableCellElement>) => (
    <td {...props} className="px-4 py-3 text-sm text-ink-700 dark:text-ink-300 border-t border-ink-100 dark:border-ink-800">{children}</td>
  ),
